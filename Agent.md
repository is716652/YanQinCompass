...
- **IAP 集成 v1 全部就位**：`utils/Purchases.ets`（queryProducts/createPurchase/queryPurchases
  云侧已购，查询失败保留缓存防降级；单机不做服务端验签的取舍已记录）+ `pages/PaywallPage.ets`
  付费墙（实时价格/买断/恢复购买/已购态）+ 时占页接入（已购不限次、未购每日 3 次+解锁入口）。
  62 断言无回归。SDK API 要点：QueryPurchasesParameter 的 productType 为**必填**。
- **沙盒联调受阻**：商品 premium_unlock（非消耗型 ¥18，AGC 状态"待提交"）与沙盒账号
  （+8619212031430）均已配置，真机购买/查询全部返回 **1001860002 The application is not
  authorized**——华为在应用级拒绝，最大嫌疑=《华为数字商品及联运服务协议》未签署完成
  （纸质合同已寄出待回签）。文档称"沙盒可先调测"，实测协议未完成时应用级授权不放行。
  **代码已就位且降级正确（免费额度+付费墙优雅失败）**；协议落定后无需改代码重测，
  预期直接通过。待用户向华为商务/工单确认 1001860002 是否需协议完成后放行。
- 商品"待提交"状态保留（首个商品须关联新版本提交审核 → 1.1.0 提审时在版本信息关联）。

### 2026-09-24：时占功能（引擎层 + UI，1.1.0 收费层核心）

- **考据**：《时占口径研究》（规则/时占口径研究.md）——时禽起法三源对照，定稿**易见元表法**
  （子时起星 = C[(曜序+元号-1)%7]，C=虚鬼箕毕氐奎翼；掌图 11 例锚点吻合，三元甲子日例题
  判整理本转录之误作存疑注记；ctext 异本互证拉丁方结构）。用户拍板：主口径/v1 范围/免费钩子。
- **引擎层**：`hour_star_table.json`（元表）+ `palace_affinity.json`（喜忌宫六组，乐/得地/忌
  分列）+ `ShizhanEngine.ets` 纯函数（起时禽/泊宫吉凶/昼夜飞伏[毕月乌昼夜皆宜特判]/旺衰）+
  DayStarUtils 暴露 `yuan`/`cycleDayIndex`。门禁 F 层 14 断言（掌图 11 例锚点、十二时禽序列
  "至井/至尾"、喜忌宫十二支覆盖），基线 46→**60 断言**。
- **UI**：`ShizhanPage.ets`（十二时辰选时[默认当下]→起盘→结果卡：时禽神兽图+宫吉/飞伏/旺衰
  徽标+白话断语+原文小字+注记，与格局卡同版式）+ `ShizhanSummary.ets` 白话组装 +
  InputPage 入口"演禽时占 · 问事择时" + main_pages 注册。**免费钩子：每日 3 次**
  （AppStorage shizhanFreeDate/Count，超出灰按钮+锁定卡，IAP 接入后升级为付费墙）。
- **真机回归**（6XE0225A31003669）：页面渲染/亥时盘（井木犴泊亥·江湖水宫·平/伏/囚）/
  午时盘（胃土雉泊午·火宫恶逆·宫逆/飞/休）/时辰切换清结果/限次锁定（0/3 灰按钮+锁定卡）全过。
- **⚠ 旺衰表囚/死互换已修正（2026-09-24）**：`seasonal_strength.json` 春/秋/冬/四季末四组
  "囚/死"两档互换（44 处，夏季原即正确）——经典定义"克我者囚、我克者死"（三命通会"春木旺火相
  水休金囚土死"），终审活例=通纂《气运说》"角亢于秋九月生者则如蛰不能变化"（秋木死档）。
  《禽星辑要》15.5 整理表春夏行的囚/死注释与档位名自相矛盾（民间口诀囚死互倒），**其宿归类可用、
  档位列不可作锚点**。修正已入库并经 61 断言门禁；装机复核待设备重连（时占页井木犴秋应显示"死"）。

### 2026-09-22：1.0.3 正式上架 + 收尾（tag 推送）

- **v1.0.3 已正式上架**（用户 2026-09-22 告知，AGC 过审）。免费层三件套+格局判定全部落地：
  图鉴 ✓ 每日禽星 ✓ 古籍阅读器 ✓ 第二案 ✓ 格局判定 ✓。
- 收尾：tag `v1.0.3-released` 打在 ef9b023 并推送 GitHub（main 同步至 ef9b023）。
- **下一版候选**（待排期）：软著登记（建议尽早，材料基于源码与古籍整理文档）；格局 v2
  （柱字要件落地需四柱干支数据、"江湖/己未"存疑条目考订、u23 鹰犬宿属考）；第三案剧本。
...
- **数据**：`rawfile/geju.json`——《通纂》福禄上格整理本实得 **37 条**（原书题三十八，缺一待考，
  不补凑）+ 贫贱下格 20 条；每条含原文注文、校勘记、白话断语、`evaluable`（19 条含柱中干支字
  要件或原文所指存疑，仅展示不自动判定——宁可漏判不可误判）、`ambiguous`（17 条含诠释，待人工核验）。
- **算法**：`utils/GejuEngine.ets` 纯函数（`init(data)` 注入 + `judge(chart)` 判定）。判定语义：
  四星泊宫绑定（主星泊命宫、胎星泊胎宫、命星泊命宫、身星泊身宫）+ 季节/昼夜/农历月全局条件；
  命中优先级主→胎→命→身，一格只报一次。
- **页面**：ResultPage 新增"格局"卡（上/下格徽标 + 白话断语 + 原文 + 注记 + 口径脚注）、
  主星卡"主星入格"徽标、专业信息"格局判定"行；口径页新增第八项"格局判定口径"。
- **引擎配套**：ChartModels 加 `lunarMonth`（格局"二八月/三月"类条件用），YanQinEngine 结果同步新增。
- **测试**：`tests/run_engine_tests.mjs` 新增 E 层 18 断言（条数守卫 上37/下20、锚点+反例 5 组、
  真实盘 200 组不变量），门禁基线 28→**46 断言**全绿。
- **核验文档**：`tests/gen_geju_doc.mjs` 从 geju.json 自动生成 `规则/格局判定-数据整理.md`
  （全量对照表 + 六项待核验要点）——文档表格永不手改，改数据重跑脚本。
- **待办**：用户核验整理文档（缺格 / 校勘四处 / 诠释 17 条 / 四星判定语义）→ 真机回归含格局卡 → 1.0.3 打包。
...
- [ ] **格局判定 v1 已实现**（2026-09-21：geju.json 57 条 + GejuEngine + E 层 18 断言 + 格局卡）——
      待用户核验 `规则/格局判定-数据整理.md`（缺格 / 校勘 / 诠释 / 判定语义）后定稿
- [ ] 格局 v2 候选：柱字要件落地（需四柱干支数据）、"江湖/己未"等存疑条目考订、上格缺的第 38 条他本补考
- [ ] 1.0.3 真机回归（选案→两案通关 / 口径页 / 每日占断 / 格局卡）→ release .app 打包 → 提审
