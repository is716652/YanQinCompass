...
- **⚠ 新发现既有数据疑点（待考据）**：`seasonal_strength.json` 秋季"相/休"归属疑似互换——
  现库 相=水系+月宿、休=土系；而标准五行定义 相=土(生我)、休=水(我生)，《禽星辑要》15.5 表
  亦相=土。囚/死两档与标准一致。影响主星与时禽在秋冬的相/休显示（1.0.1 起既有）。
  待办：对照 15.5 原表逐季核校四季数据后修 json 并补 D 层逐宿档位断言。
